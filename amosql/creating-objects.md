...
The new objects are assigned *initial values* for specified
...
```
   create Person (name,parents,income,age) 
     instances :adam ("Adam",nil,2300,64), 
                :eve ("Eve",nil,3200,63), 
...
stored functions where it is referenced. Deleted objects are printed
as:
```
#[OID nnn *DELETED*]
```
...
creation has been rolled back or its deletion committed and it is not
referenced from some variable or external system.
