...
Notice that Boolean expressions, [predicates](#predicates), either return <span style="font-style: italic;">true</span>, or nothing if the expression is not true. For example:
```
1<2 or 3<2;
 => TRUE
1<2 and 3<2;
...
Entering simple expressions followed by a semicolon is the simplest form of AmosQL [queries](#query-statement), e.g.:
```
1+sqrt(25);
```
...
The most common collection is `bags`, which are unordered sets of objects with duplicates allowed. The value of a query is usually a bag. When a query to the sa.amos toploop returns a bag as result the elements of the bag are printed on separate lines. For example:

```
sa.engine 2> select name(p) from Person p;
```
returns the bag:
```
"Bill"
...
*Vectors* are sequences of objects of any kind. Curly brackets {} enclose vector elements, for example: `set :v={1,2,3};` then `:v;` returns `{1,2,3}`.
